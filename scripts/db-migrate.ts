...
async function main() {
  ...
  if (!st.enabled) {
    console.error(`Authentication is not fully configured, so there is nothing to migrate: ${st.reason}`);
    process.exit(1);
  }
  ...
  const pool = new Pool({
    connectionString: normalizeDatabaseUrl(process.env.DATABASE_URL_UNPOOLED || process.env.DATABASE_URL),
    max: 1,
  });
  try {
