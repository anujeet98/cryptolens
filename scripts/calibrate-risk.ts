...
async function main() {
  ...
  const train = obs.filter((o) => o.i < o.n * 0.6),
    test = obs.filter((o) => o.i >= o.n * 0.6);
  console.log(
    `\n${tf}, symbols ${SYMBOLS.join("/")}: ${train.length} train obs, ${test.length} test obs (time-split 60/40 per symbol)`,
  );
  console.log(
    `x = forward range over next h bars divided by current ATR%. A forecast is q-quantile(x | label) * ATR%.\n`,
  );
  ...
  console.log(
    `ALL-DATA quantiles of x (n=${obs.length}): ` +
      HS.map((h) => {
        const a = obs.map((o) => o.x[h]).sort((p, q) => p - q);
        return `h${h}[${QS.map((q) => quantile(a, q).toFixed(3)).join(", ")}]`;
      }).join("  "),
  );
  ...
  for (const h of HS) {
    ...
    console.log(
      `train quantiles of x   ${"label".padEnd(9)} ${"n".padStart(6)}  ${QS.map((q) => `p${q * 100}`.padStart(6)).join(" ")}`,
    );
    console.log(
      `                       ${"(all)".padEnd(9)} ${String(all.length).padStart(6)}  ${QS.map((q) => quantile(all, q).toFixed(2).padStart(6)).join(" ")}`,
    );
    ...
    for (const l of labels) {
      const xs = byLabel.get(l)!;
      console.log(
        `                       ${l.padEnd(9)} ${String(xs.length).padStart(6)}  ${QS.map((q) => quantile(xs, q).toFixed(2).padStart(6)).join(" ")}`,
      );
    }
    console.log(
      `TEST (out of sample):  coverage = share of actual ranges at or below the forecast (target = q);  pinball loss, lower is better`,
    );
    console.log(
      `  ${"q".padEnd(5)} ${"cov ATR-only".padStart(13)} ${"cov +regime".padStart(12)} ${"loss ATR-only".padStart(14)} ${"loss +regime".padStart(13)} ${"improve".padStart(8)}`,
    );
    for (const q of QS) {
      ...
      for (const o of test) {
        ...
        if (y <= kAll) covA++;
        if (y <= kB) covB++;
        lossA += pinball(kAll, y, q);
        lossB += pinball(kB, y, q);
      }
      const n = test.length;
      console.log(
        `  ${String(q).padEnd(5)} ${(covA / n).toFixed(3).padStart(13)} ${(covB / n).toFixed(3).padStart(12)} ${(lossA / n).toFixed(4).padStart(14)} ${(lossB / n).toFixed(4).padStart(13)} ${(((lossA - lossB) / lossA) * 100).toFixed(1).padStart(7)}%`,
      );
    }
    console.log();
  }
