...
if (!/^[A-Z0-9]{2,20}$/.test(symbol) || !TIMEFRAMES.includes(tf)) {
  console.error(`Usage: npm run backtest -- SYMBOL ${TIMEFRAMES.join("|")} [FROM YYYY-MM-DD] [TO YYYY-MM-DD]`);
  process.exit(1);
}
...
const FEE_BPS = 4,
  SLIP_BPS = 1; // per side: roughly Binance taker fee plus a small slippage allowance
const WINDOW = 300;
...
function features(w: Candle[]): Feat | null {
  ...
  const m = computeMomentum(w),
    t = computeTechnicals(w);
  const f =
    r && m && t
      ? {
          trend: r.trend,
...
const signals: Record<string, SignalFn> = {
  ...
  "regime trend (stalled split)": (w) => {
    const f = features(w);
    return f ? `${f.trend}${f.stalled ? " (stalling)" : ""}` : null;
  },
...
function printStudy(name: string, r: StudyResult, range = false) {
  console.log(`\n== ${name}`);
  console.log(
    `${pad("label", 30, true)} ${pad("h", 3)} ${pad("n", 6)} ${pad("nEff", 6)} ${pad("mean", 8)} ${pad("hit", 6)} ${pad("excess", 8)} ${pad("t", 6)}`,
  );
  for (const label of Object.keys(r.byLabel).sort()) {
    for (const h of r.horizons) {
      ...
      console.log(
        `${pad(label, 30, true)} ${pad(h, 3)} ${pad(s.n, 6)} ${pad(Math.round(s.nEff), 6)} ${pad(range ? (s.mean * 100).toFixed(2) + "%" : pct(s.mean), 8)} ${pad(range ? "-" : (s.hitRate * 100).toFixed(0) + "%", 6)} ${pad(pct(s.excess), 8)} ${pad(s.t === null ? "-" : s.t.toFixed(1), 6)}${flag}`,
      );
    }
  }
  console.log(
    `${pad("(all bars)", 30, true)}     ${pad(r.baseline[HORIZONS[0]].n, 6)} ${pad("", 6)} ${HORIZONS.map((h) => `h${h} ${pct(r.baseline[h].mean, 3)}`).join("  ")}`,
  );
}

function row(name: string, r: SimResult) {
  console.log(
    `${pad(name, 26, true)} ${pad(pct(r.totalReturn, 1), 9)} ${pad(pct(r.buyHoldReturn, 1), 9)} ${pad(pct(r.maxDrawdown, 1), 8)} ${pad(r.trades, 6)} ${pad((r.exposure * 100).toFixed(0) + "%", 5)} ${pad(r.sharpe === null ? "-" : r.sharpe.toFixed(2), 6)} ${pad(pct(r.costPaid, 1), 8)}`,
  );
}

async function main() {
  console.log(
    `Fetching ${symbol} perp ${tf} ${new Date(fromMs).toISOString().slice(0, 10)} -> ${new Date(toMs).toISOString().slice(0, 10)} ...`,
  );
  const candles = await binance.getCandlesRange!(symbol, "perp", tf, fromMs, toMs);
  if (candles.length < WINDOW + 200) {
    console.error(`Only ${candles.length} candles: need at least ${WINDOW + 200}.`);
    process.exit(1);
  }
  let missing = 0;
  for (let i = 1; i < candles.length; i++)
    missing += Math.max(0, Math.round((candles[i].time - candles[i - 1].time) / TF_SEC[tf]) - 1);
  console.log(
    `${candles.length} candles, ${missing} missing bars, ${new Date(candles[0].time * 1000).toISOString().slice(0, 10)} -> ${new Date(candles.at(-1)!.time * 1000).toISOString().slice(0, 10)}`,
  );
  console.log(
    `Signals use the dashboard's own thresholds, no tuning. Entry at next bar's open. Costs ${FEE_BPS + SLIP_BPS} bps per side (fee ${FEE_BPS} + slippage ${SLIP_BPS}).`,
  );
  console.log(
    `h = horizon in bars; nEff = n/h (forward windows overlap); t is the excess vs all bars using nEff; * = |t| >= 3; (thin) = nEff < 30.`,
  );

  for (const [name, fn] of Object.entries(signals))
    printStudy(name, runStudy(candles, fn, { horizons: HORIZONS, window: WINDOW }));
  ...
  for (const name of ["volatility regime", "regime trend"])
    printStudy(
      `${name} -> forward range`,
      runStudy(candles, signals[name], { horizons: HORIZONS, window: WINDOW, measure: "range" }),
      true,
    );
  ...
  const desired = (rule: (f: Feat) => number) =>
    candles.map((_, i) => {
  ...
  const start = WINDOW - 1,
    mid = start + Math.floor((candles.length - 1 - start) / 2);
  console.log(
  ...
  const periods: [string, number, number][] = [
    ["FULL PERIOD", start, candles.length],
    ["FIRST HALF", start, mid + 1],
    ["SECOND HALF", mid, candles.length],
  ];
  for (const [label, a, b] of periods) {
    ...
    console.log(
      `${pad("strategy", 26, true)} ${pad("return", 9)} ${pad("hold", 9)} ${pad("maxDD", 8)} ${pad("trades", 6)} ${pad("mkt", 5)} ${pad("sharpe", 6)} ${pad("costs", 8)}`,
    );
