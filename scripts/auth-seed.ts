...
async function main() {
  ...
  if (!st.enabled) {
    console.error(`Auth is not configured: ${st.reason}`);
    process.exit(1);
  }
  if (baseUrl(process.env).startsWith("https://") && process.env.ALLOW_SEED !== "1") {
    console.error("Refusing to seed a user against an https (production-looking) URL. Set ALLOW_SEED=1 to override.");
    process.exit(1);
  }
  ...
  const user = await ctx.internalAdapter.createUser(
    { name: "Seed User", email, emailVerified: true },
    { method: "admin" },
  );
