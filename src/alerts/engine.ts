...
export const SIGNAL_INFO: Record<SignalKind, { label: string; evidence: Evidence; note: string }> = {
  "vol-extreme": { label: "Volatility turns EXTREME", evidence: "backtested", note: "Backtests: price ranges over the next hours were well above average after this state." },
  "range-spike": { label: "Unusually large recent move", evidence: "backtested", note: "The last 4 bars ranged more than the historical 90th percentile for that ATR. Says nothing about direction." },
  "liq-burst": { label: "Liquidation burst", evidence: "descriptive", note: "Liquidations in the last minute far above this coin's own average. Not backtested for what follows." },
  "funding-extreme": { label: "Funding extreme", evidence: "untested", note: "Crowded positioning is a heuristic. Not yet backtested, so treat it as context only." },
};
...
export interface SignalState { active: boolean; clearSince: number | null; lastFiredAt: number | null }
export interface EngineState { context: string | null; signals: Partial<Record<SignalKind, SignalState>> }
...
export function trailingRangePct(c: Candle[], n = RANGE_BARS): number | null {
  ...
  for (const x of w) { hi = Math.max(hi, x.high); lo = Math.min(lo, x.low); }
...
export function conditionOf(kind: SignalKind, s: Snapshot): boolean | null {
  switch (kind) {
    case "vol-extreme": return s.volatility === null ? null : s.volatility === "EXTREME";
    case "range-spike":
    ...
    case "liq-burst": return s.liqBurst === null ? null : s.liqBurst.active;
    case "funding-extreme": return s.fundingClass === null ? null : s.fundingClass === "EXTREMELY_POSITIVE" || s.fundingClass === "EXTREMELY_NEGATIVE";
  }
}
  ...
  switch (kind) {
    case "vol-extreme": {
      ...
      return {
        ...
        detail: f ? `Top 5% of recent volatility. Typical range over the next ${win}: ${f.p50.toFixed(2)}%, very wide ${f.p90.toFixed(2)}%. Direction unknown.` : "Top 5% of recent volatility. Direction unknown.",
      };
    }
    case "range-spike": {
    ...
    case "liq-burst": {
      ...
      return {
        ...
        detail: b ? `${b.dominant === "long" ? "Longs" : b.dominant === "short" ? "Shorts" : "Positions"} being flushed: ${usd(b.lastMinUsd)} in the last minute vs ${usd(b.avgMinUsd)}/min average.` : "Liquidations well above average.",
      };
    }
...
export interface EvalResult { events: AlertEvent[]; state: EngineState }
 ...
 */
export function evaluateSignals(enabled: Partial<Record<SignalKind, boolean>>, snap: Snapshot, prev: EngineState): EvalResult {
  ...
  for (const kind of kinds) {
    ...
    if (cond) {
      ...
      signals[kind] = snap.ts - since >= REARM_MS ? { ...st, active: false, clearSince: null } : { ...st, clearSince: since };
    } else {
...
export interface LevelRule { id: string; symbol: string; market: "spot" | "perp"; level: number; dir: "above" | "below"; createdAt: number }

export interface LevelResult { events: AlertEvent[]; firedIds: string[]; lastPrices: Record<string, number> }
 ...
 */
export function evaluateLevels(rules: LevelRule[], prices: Record<string, number>, lastPrices: Record<string, number>, ts: number): LevelResult {
  const events: AlertEvent[] = [];
  const firedIds: string[] = [];
  for (const r of rules) {
    const key = `${r.symbol}:${r.market}`;
    const now = prices[key], prev = lastPrices[key];
    if (now === undefined || prev === undefined) continue;
    const crossed = r.dir === "above" ? prev < r.level && now >= r.level : prev > r.level && now <= r.level;
    if (!crossed) continue;
    firedIds.push(r.id);
    events.push({
      id: `price:${r.id}:${ts}`, ts, kind: "price-cross", symbol: r.symbol,
