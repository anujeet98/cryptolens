...
export function PriceChart({
  ...
  const extras = useRef<
    {
      ...
      get: () => Series | ((c: Candle, i: number) => { value: number; color: string } | null);
    }[]
  >([]);
  ...
  useEffect(() => {
    ...
    vs.current = c.addSeries(HistogramSeries, {
  ...
  useEffect(() => {
    ...
    if (toggles.oi) {
      add("#2dd4bf", () => data.current!.oiUsd, pane, {
        lineWidth: 2,
        priceFormat: { type: "custom", minMove: 1, formatter: compactUsd },
      });
      pane++;
    }
    if (toggles.funding) {
      add("#f59e0b", () => data.current!.funding, pane, {
        lineWidth: 2,
        lineType: LineType.WithSteps,
        priceFormat: { type: "custom", minMove: 0.0001, formatter: (v: number) => `${v.toFixed(4)}%` },
      });
      pane++;
    }
    const panes = c.panes();
  ...
  useEffect(() => {
    ...
    const feed = (get: () => Series | ((c: Candle, i: number) => { value: number; color: string } | null)) => {
      ...
      return typeof g === "function"
        ? (i: number) => {
            const r = g(candles[i], i);
            return r ? { time: candles[i].time as UTCTimestamp, ...r } : null;
          }
    ...
    const mk: SeriesMarker<Time>[] = toggles.swings
      ? [
          ...pv.hi.slice(-8).map((p) => ({
            time: candles[p.index].time as UTCTimestamp,
