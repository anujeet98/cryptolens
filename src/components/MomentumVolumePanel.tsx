...
function Bar({ value }: { value: number }) {
  ...
  return (
    <div className="relative h-1.5 w-24 rounded bg-white/5">
      ...
      <div className={`absolute inset-y-0 rounded ${value >= 0 ? "bg-bull" : "bg-bear"}`} style={value >= 0 ? { left: "50%", width: `${pct}%` } : { right: "50%", width: `${pct}%` }} />
    </div>
  );
}

export function MomentumVolumePanel({ m, v, windows, ticker, tf }: {
  m: MomentumResult | null; v: VolumeAnalysis | null; windows: VolumeWindow[]; ticker: Ticker24h | null; tf: Timeframe;
}) {
  const stateCls = (s: string) => (s === "EXPANSION" ? "text-warn" : s === "CLIMAX" ? "text-squeeze" : s === "CONTRACTION" ? "text-muted" : "text-foreground");
  return (
    <section className="grid rounded border border-line bg-panel md:grid-cols-2">
      <div className="border-line p-3 md:border-r">
        ...
        {!m ? <span className="text-xs text-muted">Waiting for history…</span> : (
          <>
            <div className="flex items-end gap-4">
              <div className={`num text-3xl font-semibold ${dirCls(m.score)}`}>{m.score >= 0 ? "+" : ""}{m.score.toFixed(0)}<span className="text-sm text-muted"> / 100</span></div>
              <div className="pb-1 text-xs">
                <div className={m.trend === "DECELERATING" ? "text-warn" : m.trend === "ACCELERATING" ? "text-foreground" : "text-muted"}>{m.direction} · {m.condition}</div>
                <div className="num text-muted">acceleration {m.accelerationPts >= 0 ? "+" : ""}{m.accelerationPts.toFixed(1)} pts / 3 bars</div>
              </div>
            </div>
            ...
            <p className="mt-2 text-[11px] leading-snug text-muted">
              Composite of price-derived inputs plus independent volume confirmation. STRONG / DECELERATING means a strong move that is losing speed — not a reversal by itself.
            </p>
          </>
        )}
      </div>
      ...
      <div className="p-3">
        ...
        {!v ? <span className="text-xs text-muted">Waiting for history…</span> : (
          <div className="text-xs">
            <Row k="Current candle" v={fmtUsd(v.currentQuote)} />
            <Row k="Relative volume" v={<>{v.relativeVolume.toFixed(2)}x{v.projectedRelative !== null && <span className="text-muted"> · pace {v.projectedRelative.toFixed(2)}x</span>}</>} cls={stateCls(v.state)} />
            ...
            <div className="mt-2 border-t border-line pt-2">
              ...
              {windows.map((w) => (
                <Row key={w.label} k={`Last ${w.label}`} v={<>{fmtUsd(w.quote)} {w.ratio !== null && <span className={dirCls(w.ratio - 1)}>{w.ratio.toFixed(2)}x prev</span>}</>} />
              ))}
