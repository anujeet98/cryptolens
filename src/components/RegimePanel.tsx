...
const Kv = ({ k, v }: { k: string; v: string }) => (
  <div className="flex justify-between py-px text-xs">
  ...
  if (!r) {
    return (
      <section className="rounded border border-line bg-panel p-4 text-sm text-muted">
        Market regime: not enough candles on {tf} yet (needs 80).
      </section>
    );
  }
    ...
    <section className="rounded border border-line bg-panel">
      <div className="grid md:grid-cols-3">
        <div className="border-line p-3 md:border-r">
          ...
          <div className={`text-xl font-semibold ${t.c}`}>
            {t.t}
            {r.stalled ? <span className="ml-2 text-sm font-normal text-warn">stalling</span> : null}
          </div>
          <div className={`text-sm ${v.c}`}>
            {v.t}
            {r.volTrend !== "STEADY" ? ` · ${r.volTrend.toLowerCase()}` : ""}
          </div>
          <div className="mt-2 flex items-center gap-2 text-xs">
            <span className="text-muted">Confidence</span>
            <div
              className="h-1.5 flex-1 overflow-hidden rounded bg-line"
              title="How decisively the evidence fits this label. Not a probability of being right."
            >
              <div className="h-full bg-accent" style={{ width: `${r.confidence}%` }} />
            </div>
            <span className="num w-8 text-right">{r.confidence}%</span>
          </div>
          <div className="mt-3">
            <Title>Across timeframes</Title>
          </div>
          <div className="flex gap-1.5">
            {MTF.map((x) => {
              ...
              return (
                <div
                  key={x}
                  className={`flex-1 rounded border px-1 py-1 text-center ${x === tf ? "border-accent" : "border-line"}`}
                  title={m ? `${tt!.t} · ${VOL[m.volatility].t} · ${m.confidence}% confidence` : "loading"}
                >
        ...
        <div className="border-line p-3 md:border-r">
          <div className="mb-1 flex items-baseline justify-between">
            ...
            <span className="num text-xs text-muted">
              score {r.trendScore >= 0 ? "+" : ""}
              {r.trendScore.toFixed(0)}
            </span>
          </div>
            ...
            {r.factors.map((f) => (
              <div key={f.key} className="text-xs" title={f.detail}>
                <div className="flex justify-between">
                  <span>{f.label}</span>
                  <span className="text-[11px] text-muted">{f.detail}</span>
                </div>
                <div className="relative h-1 rounded bg-line">
                  <div className="absolute inset-y-0 left-1/2 w-px bg-muted/60" />
                  <div
                    className={`absolute inset-y-0 ${tone(f.vote)}`}
                    style={
                      f.vote >= 0
                        ? { left: "50%", width: `${f.vote * 50}%` }
                        : { right: "50%", width: `${-f.vote * 50}%` }
                    }
                  />
                </div>
              </div>
      ...
      <div className="border-t border-line px-3 py-1.5 text-[11px] text-muted">
        Rule-based read of recent price behaviour: ADX gates trend vs range, a weighted vote of seven signals sets
        direction. It describes the past and is not a forecast. Backtests (npm run backtest) found the volatility label
        does tell how much price tends to move next, but the trend and momentum labels carried no significant
        directional information. Funding/OI notes are context and never change the label.
      </div>
    </section>
  );
}
