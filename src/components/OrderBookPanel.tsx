    ...
    <>
      {rows.map((r) => (
        <div key={r.price} className="relative flex justify-between px-2 py-px text-xs">
          <div
            className={`absolute inset-y-0 right-0 ${side === "bid" ? "bg-bull/15" : "bg-bear/15"}`}
            style={{ width: `${(r.cumUsd / max) * 100}%` }}
          />
...
export function OrderBookPanel({
  ...
  const badge = stale
    ? { t: `STALE ${age}s`, c: "text-warn" }
    : ob.status === "live"
      ? { t: "LIVE", c: "text-bull" }
      : ob.status === "error"
        ? { t: "ERROR", c: "text-bear" }
        : { t: ob.status === "resyncing" ? "RESYNCING" : "SYNCING", c: "text-warn" };
  ...
  const bidTotal = bids.reduce((s, r) => s + r.usd, 0),
    askTotal = asks.reduce((s, r) => s + r.usd, 0);
    ...
    <section className="rounded border border-line bg-panel">
      <div className="grid md:grid-cols-3">
        <div className="border-line p-3 md:border-r">
          <Title>
            Order book · {symbol} {marketLabel}
          </Title>
          <div className="mb-1 flex justify-between px-2 text-[10px] uppercase text-muted">
            <span>Price</span>
            <span>Size</span>
            <span className="w-16 text-right">Cum.</span>
          </div>
          ...
          <div className="my-1 flex items-center justify-between border-y border-line px-2 py-1 text-xs">
            ...
            <span className="num text-muted">
              spread {fmtPrice(a.spread)} · {a.spreadBps.toFixed(2)} bps
            </span>
          </div>
          <Rows rows={bids} side="bid" max={max} />
          <div className="mt-1 px-2 text-[11px] text-muted">
            Top-10 imbalance{" "}
            <span className={`num ${tone(topImb)}`}>
              {topImb >= 0 ? "+" : ""}
              {(topImb * 100).toFixed(0)}%
            </span>{" "}
            · rows grouped by {fmtPrice(step)}
          </div>
        </div>
        ...
        <div className="border-line p-3 md:border-r">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-[10px] uppercase text-muted">
                ...
                <th className="text-right font-normal">Bids</th>
                <th className="text-right font-normal">Asks</th>
                <th className="text-right font-normal">Imb.</th>
              </tr>
            </thead>
            <tbody>
              {a.bands.map((b) => (
                <tr
                  key={b.pct}
                  className={b.complete ? "" : "text-muted"}
                  title={
                    b.complete
                      ? undefined
                      : "Loaded book (1000 levels) does not reach this far — value is a lower bound"
                  }
                >
                  ...
                  <td className={`num text-right ${b.complete ? tone(b.imbalance) : ""}`}>
                    {b.complete ? `${b.imbalance >= 0 ? "+" : ""}${(b.imbalance * 100).toFixed(0)}%` : "n/a"}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <p className="mt-1 text-[11px] leading-snug text-muted">
            Book reaches {a.coveragePct.bid.toFixed(2)}% below / {a.coveragePct.ask.toFixed(2)}% above mid. Grey rows
            are truncated lower bounds.
          </p>

          <div className="mt-3">
            <Title>Estimated market impact</Title>
          </div>
          <table className="w-full text-xs">
            <thead>
              <tr className="text-[10px] uppercase text-muted">
              ...
              {a.impacts.map((i) => (
                <tr key={i.usd}>
                  ...
                  {[i.buySlippageBps, i.sellSlippageBps].map((v, k) => (
                    <td key={k} className={`num text-right ${v === null ? "text-bear" : v > 30 ? "text-warn" : ""}`}>
                      {v === null ? "exceeds book" : `${v.toFixed(1)} bps`}
                    </td>
                  ))}
                </tr>
              ))}
            ...
            {a.walls.map((w) => {
              ...
              return (
                <div key={w.side + w.price} className="flex justify-between py-0.5">
                  <span className={w.side === "bid" ? "text-bull" : "text-bear"}>
                    {w.side === "bid" ? "Bid" : "Ask"} {fmtPrice(w.price)}
                  </span>
                  <span className="num text-muted">
                    {fmtUsd(w.usd)} · {w.multiple.toFixed(0)}× typical · {w.distancePct >= 0 ? "+" : ""}
                    {w.distancePct.toFixed(2)}%{active ? ` · ${Math.round(active.ageSec)}s` : ""}
                  </span>
                </div>
              );
            })}
            <p className="mt-1 text-[11px] leading-snug text-muted">
              Displayed orders can be cancelled at any time. These are only potential temporary liquidity walls, never
              confirmed intent.
            </p>
          </div>
          <div className="mt-3">
            <Title>Recent wall events</Title>
          </div>
          <div className="max-h-40 overflow-auto text-xs">
            {ob.wallEvents.length === 0 && (
              <span className="text-muted">None yet — tracking started when this book synced.</span>
            )}
              ...
              <div key={i} className="flex justify-between py-0.5">
                <span className={e.kind === "pulled" ? "text-warn" : e.kind === "consumed" ? "text-muted" : ""}>
                  {e.kind === "appeared"
                    ? "Appeared"
                    : e.kind === "pulled"
                      ? "Pulled (price never reached)"
                      : "Absorbed / filled"}{" "}
                  · {e.side}
                </span>
                <span className="num text-muted">
                  {fmtPrice(e.price)} · {fmtUsd(e.usd)}
                  {e.lifetimeSec ? ` · ${Math.round(e.lifetimeSec)}s` : ""}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="flex justify-between border-t border-line px-3 py-1.5 text-[11px] text-muted">
        <span>
          Binance · book <span className={`font-medium ${badge.c}`}>{badge.t}</span>
          {age !== null && ` · last delta ${age}s ago`} · {ob.resyncs} resync{ob.resyncs === 1 ? "" : "s"}
        </span>
