...
const OI_INTERVAL: Partial<Record<OiPeriod, string>> = {
  "5m": "5min",
  "15m": "15min",
  "30m": "30min",
  "1h": "1h",
...
async function listInstruments(category: "spot" | "linear"): Promise<Instrument[]> {
  ...
  for (let page = 0; page < 10; page++) {
    const r = await get<InstrumentList>(
      `instruments-info?category=${category}&limit=1000${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`,
    );
    out.push(...r.list);
...
export const bybit: ExchangeConnector = {
  ...
  async getCandles(symbol, marketType, tf: Timeframe, limit) {
    const r = await get<{ list: string[][] }>(
      `kline?category=${CATEGORY[marketType]}&symbol=${symbol}&interval=${INTERVAL[tf]}&limit=${Math.min(limit, 1000)}`,
    );
    const now = Date.now();
  ...
  async getTicker24h(symbol, marketType: MarketType): Promise<Ticker24h> {
    const r = await get<{ list: Record<string, string>[] }>(
      `tickers?category=${CATEGORY[marketType]}&symbol=${symbol}`,
    );
    const t = r.list[0];
  ...
  derivatives: {
    async getSnapshot(symbol): Promise<DerivativesSnapshot> {
      ...
      return {
        ...
        openInterestUsd: +t.openInterest * mark,
      };
    },
    async getFundingHistory(symbol, limit): Promise<FundingPoint[]> {
      const r = await get<{ list: { fundingRate: string; fundingRateTimestamp: string }[] }>(
        `funding/history?category=linear&symbol=${symbol}&limit=${Math.min(limit, 200)}`,
      );
      return r.list
        .map((x) => ({ time: Math.floor(Number(x.fundingRateTimestamp) / 1000), rate: +x.fundingRate }))
    ...
    async getOpenInterestHistory(symbol, period: OiPeriod, limit): Promise<OiPoint[]> {
      ...
      const [r, t] = await Promise.all([
        get<{ list: { openInterest: string; timestamp: string }[] }>(
          `open-interest?category=linear&symbol=${symbol}&intervalTime=${iv}&limit=${Math.min(limit, 200)}`,
        ),
    ...
    async getLongShortRatio(symbol, period: OiPeriod, limit): Promise<LongShortPoint[]> {
      ...
      const r = await get<{ list: { buyRatio: string; sellRatio: string; timestamp: string }[] }>(
        `account-ratio?category=linear&symbol=${symbol}&period=${iv}&limit=${Math.min(limit, 500)}`,
      );
      return r.list
        .map((x) => ({
          time: Math.floor(Number(x.timestamp) / 1000),
          ratio: +x.buyRatio / +x.sellRatio,
          longPct: +x.buyRatio * 100,
          shortPct: +x.sellRatio * 100,
        }))
        .reverse();
    },
  },
