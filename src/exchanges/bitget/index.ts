...
const LS_PERIODS: Partial<Record<OiPeriod, string>> = { "5m": "5m", "15m": "15m", "30m": "30m", "1h": "1h", "4h": "4h", "1d": "24h" };
...
export const bitget: ExchangeConnector = {
  ...
  async listMarkets() {
    ...
    const [spot, perp] = await Promise.all([
      ...
      get<{ symbol: string; baseCoin: string; quoteCoin: string; symbolStatus: string; symbolType: string }[]>(`mix/market/contracts?productType=${PRODUCT}`),
    ]);
  ...
  async getTicker24h(symbol, marketType: MarketType): Promise<Ticker24h> {
    const path = marketType === "perp" ? `mix/market/ticker?productType=${PRODUCT}&symbol=${symbol}` : `spot/market/tickers?symbol=${symbol}`;
    const t = (await get<Record<string, string>[]>(path))[0];
    if (!t) throw new Error(`Bitget: no ticker for ${symbol}`);
    return {
      exchange: "bitget", symbol, marketType, timestamp: Date.now(),
      price: +t.lastPr, changePct: +t.change24h * 100, // change24h is a fraction
      high: +t.high24h, low: +t.low24h, quoteVolume: +t.quoteVolume,
    };
  },
  ...
  derivatives: {
    async getSnapshot(symbol): Promise<DerivativesSnapshot> {
      const [tick, fund, oi] = await Promise.all([
        ...
        get<{ fundingRate: string; nextUpdate: string }[]>(`mix/market/current-fund-rate?productType=${PRODUCT}&symbol=${symbol}`),
        get<{ openInterestList: { size: string }[] }>(`mix/market/open-interest?productType=${PRODUCT}&symbol=${symbol}`),
      ]);
      const t = tick[0], f = fund[0], size = oi.openInterestList[0]?.size;
      if (!t || !f || size === undefined) throw new Error(`Bitget: incomplete derivatives data for ${symbol}`);
      const mark = +t.markPrice;
      return {
        exchange: "bitget", symbol, timestamp: Date.now(),
        markPrice: mark, indexPrice: +t.indexPrice, fundingRate: +f.fundingRate,
        nextFundingTime: Number(f.nextUpdate),
        openInterest: +size, openInterestUsd: +size * mark, // size is in base coin
      };
    },
    async getFundingHistory(symbol, limit): Promise<FundingPoint[]> {
      const rows = await get<{ fundingRate: string; fundingTime: string }[]>(`mix/market/history-fund-rate?productType=${PRODUCT}&symbol=${symbol}&pageSize=${Math.min(limit, 100)}`);
      return rows.map((r) => ({ time: Math.floor(Number(r.fundingTime) / 1000), rate: +r.fundingRate })).sort((a, b) => a.time - b.time);
    },
    ...
    async getLongShortRatio(symbol, period: OiPeriod, limit): Promise<LongShortPoint[]> {
      ...
      const rows = await get<{ longAccountRatio: string; shortAccountRatio: string; longShortAccountRatio: string; ts: string }[]>(`mix/market/account-long-short?symbol=${symbol}&period=${p}`);
      return rows.map((r) => ({ time: Math.floor(Number(r.ts) / 1000), ratio: +r.longShortAccountRatio, longPct: +r.longAccountRatio * 100, shortPct: +r.shortAccountRatio * 100 }))
