...
const EMPTY = (key: string): OrderBookState => ({ key, status: "syncing", bids: [], asks: [], analysis: null, lastEventAt: 0, resyncs: 0, wallEvents: [], activeWalls: [] });
  ...
  useEffect(() => {
    ...
    const resync = async (attempt = 0) => {
      ...
      try {
        ...
        status = "error"; error = e instanceof Error ? e.message : "order book failed";
        setTimeout(() => { syncing = false; resync(); }, 3000);
        return;
      }
      syncing = false;
    };
    ...
    const connect = () => {
      ...
      ws.onmessage = (ev) => {
        ...
        if (res === "gap" || (res === "applied" && book.isCrossed())) { resyncs++; resync(); }
      };
      ws.onclose = () => {
        if (dead) return;
        status = "resyncing"; book.reset();
    ...
    const flush = setInterval(() => {
      ...
      setSt({ key, status, bids, asks, analysis, lastEventAt, resyncs, wallEvents: tracker.events, activeWalls: tracker.active(now), error });
    }, 250);

    connect();
    return () => { dead = true; clearInterval(flush); clearTimeout(wsTimer); ws?.close(); };
  }, [symbol, market, key]);
