...
export function confidenceOf(trend: TrendRegime, adxV: number, factors: RegimeFactor[], stalled = false): number {
  ...
  if (trend === "TRANSITION")
    return Math.round(Math.min(40, 100 * (1 - Math.abs(factors.reduce((s, f) => s + f.vote * f.weight, 0) / total))));
  if (trend === "RANGE") {
...
export function classifyRegime(
  candles: Candle[],
  tf: Timeframe,
  opts: { nowMs?: number; ctx?: RegimeContext } = {},
): Regime | null {
  ...
  for (let i = Math.max(0, candles.length - 150); i < candles.length; i++) {
    const v = atrS[i];
    if (v !== null) atrPctSeries.push((v / close[i]) * 100);
  }
  const atrPercentile = percentileRank(atrPct, atrPctSeries);
  const tenAgo = atrS[candles.length - 11];
  const volTrend =
    tenAgo === null || tenAgo === undefined
      ? "STEADY"
      : atrNow / tenAgo > 1.2
        ? "EXPANDING"
        : atrNow / tenAgo < 0.8
  ...
  const add = (key: string, label: string, vote: number, weight: number, detail: string) =>
    factors.push({ key, label, vote: clamp(vote, -1, 1), weight, detail });

  add(
    "adx",
    "ADX / DI",
    Math.sign(pDI - mDI) * clamp((adxV - 15) / 30),
    3,
    `ADX ${adxV.toFixed(0)}, +DI ${pDI.toFixed(0)} / -DI ${mDI.toFixed(0)}`,
  );
  add(
    "er",
    "Efficiency",
    erDir * clamp(er / 0.6),
    1.5,
    `${(er * 100).toFixed(0)}% of the 20-bar path was net progress`,
  );
  if (tech) {
    add(
      "align",
      "MA alignment",
      tech.alignment === "BULLISH" ? 1 : tech.alignment === "BEARISH" ? -1 : 0,
      2,
      `EMA stack ${tech.alignment.toLowerCase()}`,
    );
    const e50 = tech.mas.find((m) => m.kind === "EMA" && m.period === 50);
    if (e50?.slopePct != null)
      add(
        "slope",
        "EMA50 slope",
        Math.tanh(e50.slopePct / atrPct),
        1.5,
        `${e50.slopePct >= 0 ? "+" : ""}${e50.slopePct.toFixed(2)}% over 5 bars vs ATR ${atrPct.toFixed(2)}%`,
      );
    const e200 = tech.mas.find((m) => m.kind === "EMA" && m.period === 200);
    if (e200?.distancePct != null)
      add(
        "e200",
        "Price vs EMA200",
        Math.tanh(e200.distancePct / (atrPct * 5)),
        1,
        `${e200.distancePct >= 0 ? "+" : ""}${e200.distancePct.toFixed(2)}% from EMA200`,
      );
    if (tech.macd)
      add(
        "macd",
        "MACD",
        (tech.macd.state === "BULLISH" ? 1 : -1) * (tech.macd.momentum === "WEAKENING" ? 0.5 : 1),
        1,
        `${tech.macd.state.toLowerCase()}, ${tech.macd.momentum.toLowerCase()}`,
      );
  }
  if (mom)
    add(
      ...
      mom.score / 100,
      1.5,
      `${mom.score >= 0 ? "+" : ""}${mom.score.toFixed(0)} / 100, ${mom.condition.toLowerCase()}`,
    );
  ...
  const up = trend.includes("UP"),
    down = trend.includes("DOWN"),
    trending = up || down;
  const stalled = trending && er < ER_STALL;
  if (stalled)
    notes.push(
      `ADX is still elevated from an earlier move, but the last ${ER_N} bars made almost no net progress: the ${up ? "uptrend" : "downtrend"} is pausing, not accelerating.`,
    );
  const vol = analyzeVolume(candles, tf, opts.nowMs ?? Date.now());
  const vs: VolumeState | null = vol?.state ?? null;
  if (trending && vs === "CONTRACTION")
    notes.push("The trend is running on contracting volume: less participation behind the move.");
  if (trending && vs === "CLIMAX")
    notes.push("Volume climax inside a trend: can mark exhaustion or acceleration; wait for follow-through.");
  if (trend === "RANGE" && volatility === "SQUEEZE")
    notes.push(
      "Volatility is compressed. In backtests on BTC, ETH and SOL, moves over the following hours were on average smaller than usual after a squeeze, not larger, so a breakout is not implied. Direction is unknown either way.",
    );
  if (trend === "RANGE" && volTrend === "EXPANDING")
    notes.push("Range, but volatility is rising: recent swings are widening.");
  if (volatility === "EXTREME")
    notes.push(
      "Volatility is in the top 5% of recent history. In backtests, price ranges over the following hours were well above average after this state, so stops and position sizing need extra room.",
    );
  const f = opts.ctx?.fundingClass,
    o = opts.ctx?.oiRegime;
  if (up && f === "EXTREMELY_POSITIVE")
    notes.push("Funding is extremely positive: longs are crowded, so the uptrend is vulnerable to sharp flushes.");
  if (down && f === "EXTREMELY_NEGATIVE")
    notes.push("Funding is extremely negative: shorts are crowded, so the downtrend is vulnerable to squeezes.");
  if (up && f === "EXTREMELY_NEGATIVE")
    notes.push("Uptrend against extremely negative funding: shorts are paying to stay in, which is squeeze fuel.");
  if (down && f === "EXTREMELY_POSITIVE")
    notes.push("Downtrend against extremely positive funding: longs are paying to stay in, which is flush fuel.");
  if (up && o === "NEW_LONGS") notes.push("Rising open interest with rising price: new positions support the uptrend.");
  if (up && o === "SHORT_COVERING")
    notes.push("The rise looks like short covering (OI falling), which is usually a weaker driver than new buying.");
  if (down && o === "NEW_SHORTS")
    notes.push("Rising open interest with falling price: new shorts support the downtrend.");
  if (down && o === "LONG_CLOSING")
    notes.push(
      "The decline looks like longs closing (OI falling), which usually fades once the forced exits are done.",
    );
  ...
  return {
    ...
    volTrend,
    trendScore,
    confidence: confidenceOf(trend, adxV, factors, stalled),
    adx: adxV,
    plusDI: pDI,
    minusDI: mDI,
    efficiency: er,
...
export function mtfAlignment(regimes: (TrendRegime | null | undefined)[]): MtfAlignment | null {
  ...
  const label =
    up === total
      ...
      : down === total
        ? "All timeframes trending down"
        : up > 0 && down > 0
          ? "Timeframes conflict: trends point both ways"
          : up > 0
            ? `${up} of ${total} timeframes trending up`
            : down > 0
              ? `${down} of ${total} timeframes trending down`
              : range === total
                ? "All timeframes ranging"
                : "No timeframe shows a clear trend";
