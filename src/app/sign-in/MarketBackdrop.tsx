...
export function MarketBackdrop() {
  ...
  useEffect(() => {
    ...
    let w = 0, h = 0, dpr = 1, raf = 0, offset = 0, last = 0;
    ...
    const fill = () => { while (candles.length < Math.ceil(w / STEP) + 3) candles.push(next()); };
    ...
    const draw = () => {
      ...
      for (const k of candles) { min = Math.min(min, k.lo); max = Math.max(max, k.hi); }
      const top = h * 0.18, span = h * 0.62, rng = Math.max(max - min, 1e-6);
      const y = (v: number) => top + span - ((v - min) / rng) * span;

      ctx.lineWidth = 1;
      ctx.strokeStyle = "rgba(125,134,155,0.07)";
      for (let gx = -(offset % 80); gx < w; gx += 80) { ctx.beginPath(); ctx.moveTo(gx, 0); ctx.lineTo(gx, h); ctx.stroke(); }
      ...
      ctx.beginPath();
      candles.forEach((k, i) => { const x = i * STEP - offset; if (i) ctx.lineTo(x, y(k.c)); else ctx.moveTo(x, y(k.c)); });
      ctx.shadowColor = "rgba(59,130,246,0.9)"; ctx.shadowBlur = 14;
      ctx.strokeStyle = "rgba(96,165,250,0.85)"; ctx.lineWidth = 2; ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.lineTo(candles.length * STEP - offset, h); ctx.lineTo(-offset, h); ctx.closePath();
    ...
    const move = (e: PointerEvent) => { el.style.setProperty("--mx", `${e.clientX}px`); el.style.setProperty("--my", `${e.clientY}px`); };
  ...
  return (
    <div ref={root} aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden bg-background"
      ...
      <div className="absolute inset-0" style={{ background: "radial-gradient(420px circle at var(--mx) var(--my), rgba(59,130,246,0.10), transparent 70%)" }} />
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at center, transparent 30%, rgba(10,13,20,0.85) 100%)" }} />
    </div>
  );
}
