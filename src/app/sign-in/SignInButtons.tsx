...
export function SignInButtons({ providers, next }: Props) {
  ...
  const go = async (id: string) => {
    ...
    const res = await authClient.signIn.social({
      provider: id as Social,
      callbackURL: next,
      errorCallbackURL: "/sign-in",
    });
    if (res?.error) {
      setError(res.error.message || "Could not start sign-in. Please try again.");
      setBusy(null);
    }
  };
    ...
    <div className="space-y-2.5">
      {providers.map((p) => (
        ...
        >
          ...
          <span aria-hidden className="text-muted transition group-hover:translate-x-0.5 group-hover:text-foreground">
            →
          </span>
        </button>
      ))}
      {error && (
        <p role="alert" className="pt-1 text-center text-xs text-bear">
          {error}
        </p>
      )}
    </div>
