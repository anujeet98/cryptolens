...
export function AccountPanel({ user, providers }: Props) {
  ...
  const link = async (id: string) => {
    ...
    if (res?.error) {
      setError(res.error.message || "Could not start linking.");
      setBusy(null);
    }
  };
  const unlink = async (l: Linked) => {
    setBusy(l.providerId);
    setError(null);
  ...
  const remove = async () => {
    ...
    if (res.error) {
      setError(
        res.error.message ||
          "Could not delete the account. If you signed in a while ago, sign out, sign in again and retry.",
      );
      setBusy(null);
    } else {
      router.replace("/");
      ...
      <div className="rounded-2xl border border-white/10 bg-panel/70 p-7 shadow-2xl backdrop-blur-xl">
        ...
        <ul className="mt-3 space-y-2">
          ...
          {linked?.map((l) => (
            <li
              key={l.id}
              className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm"
            >
              <Logo id={l.providerId} />
              <span className="flex-1">{label(l.providerId)}</span>
              <button
                onClick={() => unlink(l)}
                disabled={onlyOne || busy !== null}
                title={onlyOne ? "You need at least one sign-in method" : undefined}
                className="text-xs text-muted transition hover:text-bear disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:text-muted"
              >
                Unlink
              </button>
            </li>
        ...
        {error && (
          <p role="alert" className="mt-5 rounded-lg border border-bear/30 bg-bear/10 px-3 py-2 text-xs text-bear">
            {error}
          </p>
        )}
        ...
        <div className="mt-8 rounded-xl border border-bear/30 bg-bear/[0.06] p-4">
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-muted">
            Permanently removes your profile, linked sign-in methods and sessions. This cannot be undone. Type{" "}
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button
              ...
              className="rounded-lg bg-bear px-3 py-2 text-xs font-semibold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-40"
            >
