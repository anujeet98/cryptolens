...
async function handle(req: NextRequest) {
  ...
  if (!b || !Number.isInteger(b.id) || !isStatus(b.status)) return json({ error: "invalid request" }, 400);
  return (await setFeedbackStatus(b.id as number, b.status)) ? json({ ok: true }, 200) : json({ error: "not found" }, 404);
}
