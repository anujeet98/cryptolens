...
async function handle(req: NextRequest) {
  ...
  if (!b || !Number.isInteger(b.id) || !isStatus(b.status) || (b.source !== "feedback" && b.source !== "contact")) return json({ error: "invalid request" }, 400);
  return (await setInboxStatus(b.source, b.id as number, b.status)) ? json({ ok: true }, 200) : json({ error: "not found" }, 404);
}
