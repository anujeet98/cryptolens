...
const KIND = { feature: { label: "Idea", cls: "bg-accent/15 text-accent" }, bug: { label: "Bug", cls: "bg-bear/15 text-bear" }, message: { label: "Message", cls: "bg-squeeze/15 text-squeeze" }, contact: { label: "Contact", cls: "bg-warn/15 text-warn" } } as const;
...
export function AdminFeedback({ initial }: { initial: InboxRow[] }) {
  ...
  const setItemStatus = async (source: InboxSource, id: number, next: FeedbackStatus) => {
    setError(null);
    const res = await fetch("/api/admin/feedback", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ source, id, status: next }) });
    ...
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <div className="flex gap-1 rounded-lg bg-panel p-1">
          {(["all", "feature", "bug", "message", "contact"] as const).map((k) => <button key={k} onClick={() => setKind(k)} className={chip(kind === k)}>{k === "all" ? "All" : KIND[k].label} <span className="tabular-nums opacity-60">{count(k)}</span></button>)}
        </div>
      ...
      <ul className="mt-5 space-y-3">
        ...
        {shown.map((i) => (
          <li key={`${i.source}-${i.id}`} className={`rounded-xl border bg-panel p-5 ${i.status === "new" ? "border-warn/30" : "border-line"}`}>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-line pt-3 text-xs text-muted">
              ...
              {i.source === "contact" && <span className="rounded bg-white/[0.06] px-1.5 py-0.5 text-[10px] uppercase tracking-wide">from the website, not signed in</span>}
              {i.page && <span>on <span className="font-mono">{i.page}</span></span>}
              <div className="ml-auto flex gap-1">
                {STATUSES.filter((s) => s !== i.status).map((s) => <button key={s} onClick={() => setItemStatus(i.source, i.id, s)} className="rounded px-2 py-1 transition hover:bg-white/[0.06] hover:text-foreground">{s}</button>)}
              </div>
            </div>
          </li>
        ))}
