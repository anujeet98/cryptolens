...
const toSnapshot = (r: Raw): SnapshotRow => ({
  ...
  buyUsd: r.buy_usd as number,
  sellUsd: r.sell_usd as number,
  buyN: r.buy_n as number,
  sellN: r.sell_n as number,
  maxBuyUsd: r.max_buy_usd as number,
  maxSellUsd: r.max_sell_usd as number,
  mark: r.mark as number | null,
  indexPrice: r.index_price as number | null,
  fundingRate: r.funding_rate as number | null,
  oiUsd: r.oi_usd as number | null,
  mid: r.mid as number | null,
  spreadBps: r.spread_bps as number | null,
  bidUsd10bp: r.bid_usd_10bp as number | null,
  askUsd10bp: r.ask_usd_10bp as number | null,
  bidUsd50bp: r.bid_usd_50bp as number | null,
  askUsd50bp: r.ask_usd_50bp as number | null,
...
export function getSnapshots(db: DatabaseSync, symbol: string, fromSec: number, toSec: number): SnapshotRow[] {
  return (
    db
      .prepare("SELECT * FROM snapshots WHERE symbol = ? AND ts >= ? AND ts <= ? ORDER BY ts")
      .all(symbol, fromSec, toSec) as Raw[]
  ).map(toSnapshot);
}
...
export function getLiquidations(
  ...
  const rows = symbol
    ? db
        .prepare("SELECT * FROM liquidations WHERE symbol = ? AND t >= ? AND t <= ? ORDER BY t")
...
export interface Gap {
  ...
  toTs: number;
  minutes: number;
} // missing minute starts, inclusive
...
export function coverage(db: DatabaseSync, symbol: string, minGapMinutes = 2): Coverage {
  const ts = db.prepare("SELECT ts, flags FROM snapshots WHERE symbol = ? ORDER BY ts").all(symbol) as {
    ts: number;
    flags: number;
  }[];
  ...
  for (let i = 1; i < ts.length; i++) {
    ...
    if (m > 0) {
      missing += m;
      if (m >= minGapMinutes) gaps.push({ fromTs: ts[i - 1].ts + 60, toTs: ts[i].ts - 60, minutes: m });
    }
  }
  return {
    symbol,
    ...
    expectedRows: (ts[ts.length - 1].ts - ts[0].ts) / 60 + 1,
    missingMinutes: missing,
    gaps,
    partialRows: ts.filter((r) => r.flags & 1).length,
    restMissingRows: ts.filter((r) => r.flags & 2).length,
    cleanRows: ts.filter((r) => r.flags === 0).length,
  };
}
