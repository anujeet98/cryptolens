...
import { FLAG_REST_MISSING, FLAG_WS_GAP, insertLiquidations, insertSnapshots, type LiquidationRow, type SnapshotRow } from "./db";

export interface DerivSample { mark: number; index: number; fundingRate: number; oiUsd: number }
export interface BookSample { bids: [number, number][]; asks: [number, number][] }
...
export interface BookMetrics { mid: number; spreadBps: number; bidUsd10bp: number | null; askUsd10bp: number | null; bidUsd50bp: number | null; askUsd50bp: number | null }
...
export function bookMetrics(bids: [number, number][], asks: [number, number][]): BookMetrics | null {
  ...
  const b10 = band(0.1), b50 = band(0.5);
  return {
    mid: a.mid, spreadBps: a.spreadBps,
    bidUsd10bp: b10?.complete ? b10.bidUsd : null, askUsd10bp: b10?.complete ? b10.askUsd : null,
    bidUsd50bp: b50?.complete ? b50.bidUsd : null, askUsd50bp: b50?.complete ? b50.askUsd : null,
  };
}
...
export class Recorder {
  ...
  constructor(private db: DatabaseSync, readonly symbols: string[], private deps: RecorderDeps, nowMs = Date.now()) {
    for (const s of symbols) this.agg.set(s, new MinuteAggregator());
    const r = db.prepare("INSERT INTO runs (started, heartbeat, symbols) VALUES (?,?,?)").run(Math.floor(nowMs / 1000), Math.floor(nowMs / 1000), symbols.join(","));
  ...
  onLiquidation(e: LiquidationRow): void { this.liqBuf.push(e); }

  markDown(nowMs: number): void { for (const a of this.agg.values()) a.markDown(nowMs); }
  markUp(nowMs: number): void { for (const a of this.agg.values()) a.markUp(nowMs); }
  ...
  async tick(nowMs: number): Promise<number> {
    ...
    try {
      ...
      await Promise.all([...flushed].map(async ([symbol, minutes]) => {
        ...
        let d: DerivSample | null = null, bm: BookMetrics | null = null;
        if (needsRest) {
          const [dr, br] = await Promise.allSettled([this.deps.sampleDerivs(symbol), this.deps.sampleBook(symbol)]);
          if (dr.status === "fulfilled") d = dr.value; else this.deps.log?.(`${symbol} derivatives sample failed: ${dr.reason instanceof Error ? dr.reason.message : dr.reason}`);
          if (br.status === "fulfilled") bm = bookMetrics(br.value.bids, br.value.asks); else this.deps.log?.(`${symbol} order book sample failed: ${br.reason instanceof Error ? br.reason.message : br.reason}`);
        }
        for (const m of minutes) {
          const isLatest = m.ts === latest;
          const dd = isLatest ? d : null, bb = isLatest ? bm : null;
          rows.push({
            symbol, ts: m.ts, open: m.open, high: m.high, low: m.low, close: m.close,
            buyUsd: m.buyUsd, sellUsd: m.sellUsd, buyN: m.buyN, sellN: m.sellN, maxBuyUsd: m.maxBuyUsd, maxSellUsd: m.maxSellUsd,
            mark: dd?.mark ?? null, indexPrice: dd?.index ?? null, fundingRate: dd?.fundingRate ?? null, oiUsd: dd?.oiUsd ?? null,
            mid: bb?.mid ?? null, spreadBps: bb?.spreadBps ?? null,
            bidUsd10bp: bb?.bidUsd10bp ?? null, askUsd10bp: bb?.askUsd10bp ?? null, bidUsd50bp: bb?.bidUsd50bp ?? null, askUsd50bp: bb?.askUsd50bp ?? null,
      ...
      const liqs = this.liqBuf; this.liqBuf = [];
