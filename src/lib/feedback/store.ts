...
export interface InboxRow {
  source: InboxSource;
  id: number;
  kind: "feature" | "bug" | "message" | "contact";
  title: string;
  body: string;
  page: string;
  status: string;
...
export async function listInbox(limit = 200): Promise<InboxRow[]> {
  const [f, c] = await Promise.all([
    db().query(
      `select f.id, f.kind, f.title, f.body, f.page, f.status, f."createdAt", u.name, u.email
                from feedback f join "user" u on u.id = f."userId" order by f."createdAt" desc limit $1`,
      [limit],
    ),
    db().query(
      `select id, name, email, message as body, status, "createdAt" from contact order by "createdAt" desc limit $1`,
      [limit],
    ),
  ]);
  const rows: InboxRow[] = [
...
export async function addContact(
  ...
  const r = await db().query(
    ...
    [
      ...
      CONTACT_LIMITS.perHourPerIp,
      CONTACT_LIMITS.perDayTotal,
    ],
  );
  return (r.rowCount ?? 0) > 0;
}
