...
export interface FeedbackInput { kind: FeedbackKind; title: string; body: string; page: string }
...
export function parseFeedback(raw: unknown): Parsed {
  ...
  const kind = r.kind;
  if (typeof kind !== "string" || !(KINDS as readonly string[]).includes(kind)) return { ok: false, error: "Choose what kind of feedback this is." };
  ...
  if (title.length > LIMITS.titleMax) return { ok: false, error: `Keep the title under ${LIMITS.titleMax} characters.` };
  ...
  const page = typeof r.page === "string" && r.page.startsWith("/") && !r.page.startsWith("//") ? r.page.split(/[?#]/)[0].slice(0, LIMITS.pageMax) : "";
