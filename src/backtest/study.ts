...
export function runStudy(candles: Candle[], signal: SignalFn, opts: StudyOptions): StudyResult {
  ...
  for (let i = warm - 1; i < candles.length - 1; i++) {
    ...
    for (const h of hs) {
      ...
      if (label !== null) {
        ...
        if (!g) {
          g = Object.fromEntries(hs.map((x) => [x, [] as number[]]));
          groups.set(label, g);
        }
        g[h].push(r);
      }
  ...
  for (const h of hs)
    baseline[h] = {
      ...
      mean: mean(fwd[h]),
      std: std(fwd[h]),
      hitRate: fwd[h].length ? fwd[h].filter((x) => x > 0).length / fwd[h].length : 0,
    };
  ...
  for (const [label, g] of groups) {
    ...
    for (const h of hs) {
      ...
      byLabel[label][h] = {
        ...
        median: median(xs),
        std: sd,
        hitRate: n ? xs.filter((x) => x > 0).length / n : 0,
        excess,
        t: nEff >= 2 && sd > 0 ? excess / (sd / Math.sqrt(nEff)) : null,
        reliable: nEff >= MIN_EFF,
      };
    }
  }
