...
export const OI_WINDOWS = [
  { label: "5m", sec: 300 }, { label: "15m", sec: 900 }, { label: "1h", sec: 3600 }, { label: "4h", sec: 14400 }, { label: "24h", sec: 86400 },
] as const;
...
export const REGIME_TEXT: Record<OiRegime, { title: string; text: string }> = {
  ...
  SHORT_COVERING: { title: "Price ↑ · OI ↓", text: "Likely short covering / short liquidations rather than fresh buying." },
...
export function interpretOi(priceChangePct: number, oiChangePct: number): OiRegime {
  const priceFlat = Math.abs(priceChangePct) < 0.1, oiFlat = Math.abs(oiChangePct) < 0.3;
...
function at(hist: OiPoint[], targetSec: number): OiPoint | null {
  ...
  for (const p of hist) { if (p.time <= targetSec) best = p; else break; }
  return best;
}
...
export function analyzeOi(hist: OiPoint[], snap: DerivativesSnapshot): OiAnalysis | null {
  ...
  return {
    oi: snap.openInterest, oiUsd: snap.openInterestUsd, windows,
