...
function at(x: Ctx, i: number): { score: number; comps: MomentumComponent[] } | null {
  ...
  const body =
    last5.reduce((s, k) => s + (k.close - k.open), 0) / (last5.reduce((s, k) => s + (k.high - k.low), 0) || 1);
  let run = 0;
  for (let j = i; j > i - 8; j--) {
  ...
  const comps: MomentumComponent[] = [
    {
      key: "roc",
      label: "Rate of change (10)",
      value: sq(roc * 1.5),
      weight: W.roc,
      detail: `${((px / c[i - 10].close - 1) * 100).toFixed(2)}%`,
    },
    {
      key: "accel",
      ...
      value: sq(accel * 2),
      weight: W.accel,
      detail: accel >= 0 ? "speeding up" : "slowing",
    },
    ...
    {
      key: "body",
      label: "Candle bodies (5)",
      value: clamp(body * 1.3),
      weight: W.body,
      detail: `${(body * 100).toFixed(0)}% of range`,
    },
    {
      key: "run",
      ...
      value: clamp(run / 5),
      weight: W.run,
      detail: run === 0 ? "none" : `${Math.abs(run)} ${run > 0 ? "green" : "red"}`,
    },
    {
      key: "ema",
      label: "Distance from EMA20",
      value: sq((px - e) / a / 3),
      weight: W.ema,
      detail: `${(((px - e) / e) * 100).toFixed(2)}%`,
    },
    {
      key: "vwap",
      label: "Distance from VWAP",
      value: v === null ? 0 : sq((px - v) / a / 3),
      weight: W.vwap,
      detail: v === null ? "n/a" : `${(((px - v) / v) * 100).toFixed(2)}%`,
    },
    {
      key: "volume",
...
export function computeMomentum(c: Candle[]): MomentumResult | null {
  ...
  return {
    ...
    strength,
    trend,
    condition: `${strength} / ${trend}`,
    components: now.comps,
  };
}
