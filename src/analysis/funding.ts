...
export function analyzeFunding(
  ...
  return {
    ...
    previous: prev,
    change: prev === null ? null : currentRate - prev,
    percentile7d: percentileOf(rate8h, w7),
    percentile30d: p30,
    samples7d: w7.length,
    samples30d: w30.length,
...
export function fundingContext(cls: FundingClass, priceChangePct: number): string | null {
  const up = priceChangePct > 0.3,
    down = priceChangePct < -0.3;
  if (up && cls === "EXTREMELY_NEGATIVE")
    return "Price rising while funding is very negative: shorts may be crowded — consistent with squeeze risk.";
  if (up && cls === "EXTREMELY_POSITIVE")
    return "Price rising with very positive funding: longs look crowded — the move may be fragile.";
  if (down && cls === "EXTREMELY_POSITIVE")
    return "Price falling while funding is very positive: longs may be trapped.";
  if (down && cls === "EXTREMELY_NEGATIVE")
    return "Price falling with very negative funding: shorts are crowded — bounce/squeeze risk if selling stalls.";
  if (cls === "EXTREMELY_POSITIVE" || cls === "EXTREMELY_NEGATIVE")
    return "Funding is extreme, but funding alone is not a reversal signal.";
  return null;
}
