...
export interface Technicals {
  ...
  vwap: {
    day: number | null;
    week: number | null;
    dayPosition: "above" | "below" | null;
    event: "reclaim" | "rejection" | null;
  };
}
...
export function computeTechnicals(c: Candle[]): Technicals | null {
  ...
  const mas = [
    ...smas.map(([p, s]) => maState("SMA", p, s, price)),
    ...emas.map(([p, s]) => maState("EMA", p, s, price)),
  ];
  ...
  const rsiNote =
    rsiZone === "OVERBOUGHT"
      ? "Overbought ≠ immediate reversal — strong trends can stay overbought for a long time."
      : rsiZone === "OVERSOLD"
        ? "Oversold ≠ immediate bounce — strong downtrends can stay oversold for a long time."
        : null;
  ...
  if (bw[bw.length - 1] !== null) {
    ...
    bbOut = {
      ...
      lower: last(bb.lower)!,
      bandwidth: cur,
      percentB: pb[pb.length - 1]!,
      volatility:
        prev !== null && cur > prev * 1.15
          ? "EXPANDING"
          : prev !== null && cur < prev * 0.87
            ? "CONTRACTING"
            : "NORMAL",
      squeeze: recent.length >= 50 && rank <= 0.1,
      walking:
        tail.length === 4 && tail.every((x) => x !== null && x > 0.95)
          ? "upper"
          : tail.length === 4 && tail.every((x) => x !== null && x < 0.05)
            ? "lower"
            : null,
    };
  }
